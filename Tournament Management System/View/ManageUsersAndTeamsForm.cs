...
namespace Tournament_Management_System.View
{
    public partial class ManageUsersAndTeamsForm : Form
    {
        ...
        private void deleteTeamButton_Click(object sender, EventArgs e)
        {
            ...
            PlayerController pc = new PlayerController();
            int playerCount = pc.GetPlayerCountByTeam(teamId);

            if (playerCount > 0)
            {
                MessageBox.Show("Cannot delete this team because it still has players assigned to it. Please remove all players from the team first.", "Deletion Failed", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
